...
"""
在数据预处理时，尽量输入的特征时不相关且满足一个标准的正态分布，这样的模型表现也较好
但对于深层的神经网络，网络的非线性层会使得输出的结果变得相关，且不再满足一个标准的N(0,1)分布，这使得训练变得困难。
批标准化就是对每一层网络的输出，对其做一个归一化，使其服从标准的正态分布，这样后一层网络的输入也是一个标准的正态分布，能够较好的进行训练，加快收敛速度
"""
